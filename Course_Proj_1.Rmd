...
##1. Code for reading in the dataset and/or processing the data
```{r, echo = TRUE}
setwd("C:\\Users\\TRUDR\\OneDrive - Monsanto\\Migrated from My PC\\Desktop\\Data\\Reprod.Res\\course_proj_1")
...
#Transform the date column into date format using lubridate
library(lubridate)
...
```{r, echo = TRUE}
###Assuming that the words on average means averaging steps by date and interval
...
##6.Code to describe and show a strategy for imputing missing data
The common strategies used for imputing missing data include:
1. Constant value imputations
2. Regression model value imputations
3. Mean/mode value substitutions
To answer this question, I use mean/mode value substitution strategy to impute missing values, i.e., using mean values to substitute the missing values in the original data set
Also try to understand the distribution of missing values before imputation
...
We can observe a distict pattern of missing values as shown by the plot such as there are 8 missing values for each interval. There are about 288 missing values for atotal of 8 dates

```{r, echo = TRUE}
###Dates having missing values 
library(lubridate)
...
###Simplification of the imputed data frame
